    ...
    def _packet_in_handler(self, ev):
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if eth.ethertype == ether_types.ETH_TYPE_IP:
                ...
                if self.mitigation:
                    #self.logger.info("Mitigation mode active - checking traffic")
                    if not (srcip in self.arp_ip_to_port[dpid][in_port]):
                        #self.logger.info("Attack detected from switch: sw%s", dpid)
                        #self.logger.info("Attack detected from port %s", in_port)            
                        #self.logger.info("Blocking port %s", in_port)
